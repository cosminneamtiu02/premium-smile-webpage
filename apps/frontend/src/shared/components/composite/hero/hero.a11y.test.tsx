...
describe("Hero — a11y", () => {
  it("has no axe violations with multiple slides", async () => {
    const { container } = renderWithProviders(
      <Hero slides={[SLIDE_A, SLIDE_B]} ctaPrimary={PRIMARY} ctaSecondary={SECONDARY} />,
    );
  ...
  it("has no axe violations with a single slide", async () => {
    const { container } = renderWithProviders(
      <Hero slides={[SLIDE_A]} ctaPrimary={PRIMARY} ctaSecondary={SECONDARY} />,
    );
    expect(await axe(container)).toHaveNoViolations();
  });
  ...
  it("has no axe violations with an empty slides array", async () => {
    const { container } = renderWithProviders(
      <Hero slides={[]} ctaPrimary={PRIMARY} ctaSecondary={SECONDARY} />,
    );
    expect(await axe(container)).toHaveNoViolations();
  });
});
